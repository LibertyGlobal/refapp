...
function requestJson(options, headers) {
    ...
    if (options.body !== undefined) {
        config.headers = Object.assign(config.headers, { 'Content-Length': Buffer.byteLength(options.body) });
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        const request = http.request(requestorConfig, (response) => {
            ...
            response.on('data', (chunk) => {
                ...
                log.info('Request:data', responseLogPrefix, `chunk.length: "${chunk.length}", currentContentLength: "${currentContentLength}"`);
            });
            ...
            response.on('end', () => {
                ...
                if (response.headers['content-type'] && response.headers['content-type'].includes('json')) {
                    try {
                        ...
                        log.error(MODULE_NAME, responseLogPrefix, `Error parsing response: "${error}" for response "${result}"`);
                ...
                if (isResponseSuccess(response)) {
                    ...
                    log.warn(MODULE_NAME, responseLogPrefix, `Warning, response status received: ${response.statusCode}`);
                    reject(new Error({ statusCode: response.statusCode, data: result }));
                }
            });
        });
