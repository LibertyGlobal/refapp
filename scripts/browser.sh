...
function stop_browser()
{
  if [ -f /etc/WPEFramework/plugins/RDKShell.json ]; then
    ...
    curl -X PUT http://127.0.0.1:9998/Service/Controller/Deactivate/WebKitBrowser
  fi
  exit
}
...
if [ -f /etc/WPEFramework/plugins/RDKShell.json ]; then
  ...
  curl -X PUT http://127.0.0.1:9998/Service/Controller/Activate/WebKitBrowser
  curl -d "{\"url\":\"$1\"}" http://127.0.0.1:9998/Service/WebKitBrowser/URL
fi
